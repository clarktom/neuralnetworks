...
for epoch in range(training_epochs):
    ...
    for start, end in zip(range(0, len(trX), batch_size), range(batch_size, len(trX), batch_size)):
        tilde_trX = theano_rng.binomial(size=trX[start:end].shape, n=1, p=1 - corruption_level,
                                     dtype=theano.config.floatX)*trX[start:end]
        cost = train_da1(tilde_trX[start:end])
        c.append(cost)
